...
{
    float proportional_angular_gain = 0.05;
...
/*     std::array<pose, num_trajectory_points>goal = {{1,1, M_PI/2},
   ...
   // pub.publish(velocity_command);
    ...
    while(ros::ok() && (goal_point < num_trajectory_points))
    {
        ...
        while(pose_error > 0.1 && counter < 15)
        {
            float desired_heading = (M_PI + atan2(goal[goal_point].y, goal[goal_point].x));
            while(abs(orientation_error) > 0.1 )
            {

                orientation_error = current_turtle_pose.orientation - desired_heading;
